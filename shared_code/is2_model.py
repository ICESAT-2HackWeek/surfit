...
def is2modelfit_nonnormalized(xdata,p,range_bins_m_hr):
	...
	if p[0] == 0:  #If user specified function is Gaussian, then recalculate
		sig_surf = p[2]
		surf_pdf = 1/(np.sqrt(2*np.pi)*sig_surf) * np.exp(-0.5*(taufit_hr/sig_surf)**2)
		surf_pdf = surf_pdf/max(surf_pdf)
	...
	#surf_pdf2 = surf_pdf2/np.sum(surf_pdf2)
	...
	power_final = power_final/np.sum(power_final)  #*p[0]
